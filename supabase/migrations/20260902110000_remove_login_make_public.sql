-- Remove the login restriction entirely: this app goes back to being a
-- single shared, public workspace with no authentication required, per the
-- original design ("anyone who opens the published link should see the
-- exact same live dataset").
